...
# 2. hidapi (Linux only; macOS uses IOKit natively)
...
if [[ "$PLATFORM" == linux-* ]]; then
    cp "$BUILD_DIR/hidapi-install-$PLATFORM/lib/libhidapi-libusb.a" "$OUT/lib/" 2>/dev/null || \
        cp "$BUILD_DIR/hidapi-install-$PLATFORM/lib/libhidapi-hidraw.a" "$OUT/lib/" 2>/dev/null || true
fi
